...
public class MyTreeUtils {
    ...
    /**
     * 题目：给定一棵二叉树和其中一个结点，如何找出中序遍历顺序的下一个结点》
     * 树中的结点除了有两个分别指向左右子结点的指针以外，还有一个指向父结点的指针。
     */
    /*
     * 思路：1)当pNode有右子树时，下一个结点为右子树最左边的结点；
     * 2)当pNode没有右子树，它是其父结点的左孩子时，下一个结点为父结点；
     * 3)当以上两种情况都不是时，需要向上逆向遍历，直到发现pNode的某个祖先结点为该结点父结点的左孩子时，
     * 下一个结点就是该祖先结点的父结点。
     */
    public static TreeNodeWithParent getNext(TreeNodeWithParent pNode)
    {
        if (pNode == null || pNode.parent == null) {// 空树
            return null;
        }
        TreeNodeWithParent result = null;
        TreeNodeWithParent temp = null;
        
        if (pNode.right != null) {// 如果pNode有右子树
            temp = pNode.right;
            while (temp.left != null) {// 定位到右子树最左结点
                temp = temp.left;
            }
            result = temp;
        } else {// pNode无右子树
            temp = pNode.parent;
            while (temp != null && temp.left != pNode) {// pNode不是树的根节点并且它的父结点不是父结点的左孩子
                pNode = temp;
                temp = pNode.parent;
            }
            result = temp;
